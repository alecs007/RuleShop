...
export function getFactOptions(): FactOption[] {
  return FACTS.map(({ path, label, type, example }) => ({ path, label, type, example }));
}
...
export function getActionOptions(category: DecisionCategory): ActionOption[] {
  return actionsForCategory(category).map(({ type, label, params }) => ({
    type,
    label,
    params: params.map((p) => ({ ...p, oneOf: p.oneOf ? [...p.oneOf] : undefined })),
  }));
}
