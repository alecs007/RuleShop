/**
 * Contractul de filtrare al catalogului — partajat intre pagina de produse
 * (server) si panoul de filtre (client). Toata starea trăiește in query params,
 * deci orice combinatie de filtre e shareable prin URL si respecta back/forward.
 *
 * Fisierul NU importa Prisma sau `server-only`: traducerea in `where` se face
 * in `lib/shop/products.ts`.
 */
...
/** Selectia de filtre, normalizata. Praguri de preț in lei (unitatea majora). */
...
/** Fereastra pentru filtrul „noutăți”. */
...
/**
 * Citeste o lista de valori acceptand ambele forme intalnite in URL-uri:
 * repetarea cheii (`?tag=a&tag=b`) si lista separata prin virgula (`?tag=a,b`).
 */
...
/** Numar pozitiv sau `null` — valorile aberante sunt ignorate, nu erori. */
...
export function parseCatalogSelection(params: RawParams): CatalogSelection {
  ...
  // Interval inversat de utilizator: il ordonam in loc sa returnam zero produse.
...
/** Doar ce difera de starea implicita ajunge in URL. */
...
/** Cate filtre sunt active — pentru contorul de pe butonul de filtrare. */
...
/** Comuta o valoare intr-o lista de filtre, pastrand ordinea de intrare. */
