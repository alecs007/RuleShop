...
import { CATALOG_GRID_CLASSES } from "@ruleshop/storefront";
...
export default async function ProductsPage({
  ...
  const [{ store: segment }, query] = await Promise.all([params, searchParams]);
  const store = await requireStore(segment);
  const selection = parseCatalogSelection(query);

  // Products hidden by AVAILABILITY rules leave the catalog before pagination
  // and the counts, as if they did not exist.
  ...
  const [prices, availability, theme] = await Promise.all([
    ...
    // The same per-request THEME evaluation as the layout; here it only
    // decides the grid density.
  ...
  return (
    <div className="appear-content py-8">
      ...
      {result.products.length === 0 ? (
        ...
        <AppearList
          // Any change of filter, sort or page replays the card entrance.
