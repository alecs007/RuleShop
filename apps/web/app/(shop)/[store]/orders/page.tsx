...
/**
 * The current visitor's orders, account or not: guest orders are found through
 * the session cookie, and stay in the same list once an account is created.
 */
    ...
    <div className="appear-content mx-auto max-w-2xl py-8">
      ...
      {!viewer && orders.length > 0 && (
        <p className="mt-1 text-sm text-ink-muted">
          ...
          <Link
            href={`/auth/signin?callbackUrl=${encodeURIComponent(storeHref(store.pathPrefix, "/orders"))}`}
        ...
        <ul className="mt-6 space-y-3">
          {orders.map((order) => {
            ...
            return (
              <li key={order.id}>
                <Link
                  href={storeHref(store.pathPrefix, `/orders/${order.orderNumber}`)}
